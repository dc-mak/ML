...
fun all_pathq pred tree = let

    fun find (Tw a) sum path rest =
        if pred a then Cons(rev (a::path), rest) else rest()

      | find (Bv(a, left, right)) sum path rest = let

          val sum2 = sum+a and path2 = a::path
          val rstR = fn () => find right sum2 path2 rest
          and rstLR = fn () => find left sum2 path2 rest

        in if pred sum2 then Cons(rev path2, rstLR) else rstLR() end

  in find tree 0 [] (fn () => Nil) end;
...
fun all_ps_q p t = let

    fun find (Tw a) sum path =
        if (p (sum+a)) then Cons(rev (a::path), fn () => Nil) else Nil

      | find (Bv(a,tl,tr)) sum path = let

        val sum2 = sum+a
        val path2 = a::path
        val rest = inter (find tl sum2 path2) (find tr sum2 path2)

      in if (p sum2) then Cons(rev path2, fn () => rest) else rest end
