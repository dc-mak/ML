...
fun arrayoflist [] = Lf
  ...
  | arrayoflist (x::xs) = let

    val even = altern xs
    val odd = altern (tl xs)

  in Br(x, arrayoflist even, arrayoflist odd) end;
...
fun check pred arr = let

  fun chk pred n Lf = []
    | chk pred n (Br(x, l, r)) = let

      val even = chk pred (2*n) l
      val odd = chk pred (2*n+1) r
      val result = merge even odd

    in if pred n then x::result else result end

in chk pred 1 arr end;
