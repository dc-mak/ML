...
let
  fun gen xs 0 r = r
    | gen xs m r =
    gen xs (m-1) ((map (fn x => x*m) xs)::r)
in
  gen (List.tabulate(n, fn i => i+1)) n []
end;
...
in
  tab(n, fn i => tab(n, fn j => tab (n, fn k => f i j k)))
end;
